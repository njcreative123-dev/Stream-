...
p.on('console', m => { if (m.type()==='error'||m.type()==='warning') console.log('CONSOLE', m.type(), m.text().slice(0,160)); });
...
console.log('log =', (await p.evaluate(() => document.getElementById('log').textContent)).slice(0,500));
await b.close();
