// src/workers/working_urls.mjs
var WORKING_URLS = /* @__PURE__ */ new Set([
  ...
  "http://202.70.146.135:8000/play/a02n/index.m3u8",
  "http://202.70.146.135:8000/play/a02t/index.m3u8",
  "http://202.70.146.135:8000/play/a04p/index.m3u8",
  "http://202.70.146.135:8000/play/a058/index.m3u8",
  "http://202.70.146.135:8000/play/a06c/index.m3u8",
  "http://59.103.38.46:8000/play/a052/index.m3u8",
  "http://59.103.38.46:8000/play/a05x/index.m3u8",
  ...
  "https://amg00644-amg00644c1-ondemandkorea-amesia-10266.playouts.now.amagi.tv/playlist/amg00644-tvtodaynetworkltdfast-indiatoday-ondemandkoreaamesia/playlist.m3u8",
  ...
  "https://cloudplay-sonyliv.pages.dev/ten2hd.m3u8",
  ...
  "https://cloudplay-sonyliv.pages.dev/ten4hd.m3u8",
  "https://cloudplay-sonyliv.pages.dev/ten5.m3u8",
  "https://cloudplay-sonyliv.pages.dev/ten5hd.m3u8",
...
var working_urls_default = WORKING_URLS;
...
var JWT_SECRET = "njstream-secret-7f3a9c2e1b8d4";
...
async function initAuthTable(db) {
  try {
    await db.prepare("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE NOT NULL, email TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, role TEXT DEFAULT 'user', prime_until INTEGER DEFAULT 0, avatar TEXT DEFAULT '\u{1F464}', created_at DATETIME DEFAULT CURRENT_TIMESTAMP)").run();
    const admin = await db.prepare("SELECT id FROM users WHERE username = ?1").bind("admin").first();
    if (!admin) {
      await db.prepare("INSERT INTO users (username, email, password_hash, role, avatar) VALUES (?, ?, ?, ?, ?)").bind("admin", "admin@njstream.dev", simpleHash("admin123"), "admin", "\u{1F451}").run();
    }
  } catch (e) {
  }
}
...
async function handleRegister(request, env) {
  ...
  try {
    ...
    const avatar = username === "admin" ? "\u{1F451}" : "\u{1F464}";
...
function tgThumb(title, cat) {
  const palettes = {
    videos: ["#7c3aed", "#2563eb"],
    books: ["#059669", "#0d9488"],
    apk: ["#d97706", "#dc2626"],
    audios: ["#db2777", "#7c3aed"],
    photos: ["#2563eb", "#06b6d4"],
    texts: ["#4f46e5", "#9333ea"],
    docs: ["#475569", "#334155"],
    archives: ["#92400e", "#a16207"],
    other: ["#334155", "#0f172a"]
  };
  const icons = { videos: "\u{1F3AC}", books: "\u{1F4DA}", apk: "\u{1F4E6}", audios: "\u{1F3B5}", photos: "\u{1F5BC}\uFE0F", texts: "\u{1F4AC}", docs: "\u{1F4C4}", archives: "\u{1F5DC}\uFE0F", other: "\u{1F4C1}" };
...
async function buildTgLibrary(env, chatId) {
  ...
  try {
    ...
    for (const k of listed.keys) {
      ...
      try {
        ...
        mirrors.set(k.name.replace("media:ext:", ""), d);
      } catch (e) {
      }
    }
  } catch (e) {
  ...
  for (const it of items) {
    ...
    if (prev) {
      if ((it.file ? it.file.size : 0) > (prev.file ? prev.file.size : 0)) {
        seen.set(key, it);
      }
      continue;
    }
  ...
  const lib = { built: Date.now(), chatId, total: deduped.length, items: deduped, categories };
...
async function handleTelegramLibrary(url, env) {
  ...
  return json({ ok: true, cat, q, sort, total: items.length, counts, items: items.slice(0, limit), built: lib.built || 0 });
}
    ...
    if (env.KV_STORE) {
      ...
      if (chunk) {
        ...
        if (fmt === "webm") {
          ...
          try {
            const mt = JSON.parse(metaText);
            if (mt.type) contentType = mt.type;
            if (mt.name) name = mt.name;
          } catch (e) {
          }
        }
        const total = chunk.byteLength || 0;
        if (probe) return json({ available: true, source: "kv", size: total, sizeLabel: sizeLabelB(total), name, mime: contentType });
        const range = request.headers.get("Range");
        const headers = {
...
async function handleMediaRegister(request, env) {
  ...
  try {
    await env.KV_STORE.delete("media:req:" + id);
  } catch (e) {
  }
...
async function handleMediaMirrors(request, env) {
  ...
  for (const k of listed.keys) {
    ...
    try {
      const d = JSON.parse(raw);
      out.push({ id: k.name.replace("media:ext:", ""), url: d.url, name: d.name, mime: d.mime, size: d.size || 0, sizeLabel: sizeLabelB(d.size || 0), source: d.source || "ext", registered: d.registered || 0 });
    } catch (e) {
    }
  }
  out.sort(function(a, b) {
...
async function handleR2Import(request, url, env) {
  ...
  try {
    if (ct.includes("application/json")) {
      ...
      await env.MEDIA_BUCKET.put(key2, upstream.body, { httpMetadata: { contentType: body.mime || "video/mp4", contentDisposition: "inline" } });
      ...
      return json({ ok: true, id: finalId, key: key2 });
    }
...
async function handleR2List(request, env) {
  ...
  return json({ objects: listed.objects.map(function(o) {
    return { key: o.key, size: o.size, uploaded: o.uploaded };
  }), truncated: listed.truncated, usage: listed.objects.reduce(function(t, o) {
    return t + o.size;
  }, 0) });
}
...
async function handleTelegramIngest(request, env) {
  ...
  for (const msg of body.messages) {
    ...
    if (!index.ids.includes(entry.id)) {
      index.ids = [entry.id, ...index.ids].slice(0, 2e4);
...
async function handleLiveTV(url, env) {
  ...
  if (env.KV_STORE) {
    try {
      ...
      if (cached && cached.channels?.length > 0) {
        ...
        return json({ ...cached, channels: cch, total: cch.length, working: cch.filter((c) => c.working).length, hindi: cch.filter((c) => c.hindi).length, onlyWorking });
      }
    } catch (e) {
    }
  }
  ...
  for (const ch of allChannels) {
    ...
    ch.working = working_urls_default.has(key);
    merged.push(ch);
  }
  merged.sort((a, b) => {
...
async function proxyLiveTV(request, url) {
  ...
  if (isHls) {
    ...
    const rewritten = full.split("\n").map((line) => {
      ...
      let abs;
      try {
        abs = new URL(t, parsed.href).href;
      } catch (e) {
        return line;
      }
...
function generateFallbackMovies(type) {
  const ALL = [
    ...
    { id: 2, title: "Pathaan", overview: "An Indian spy takes on a ruthless enemy.", image: "", rating: 7, year: "2023" },
    ...
    { id: 11, title: "Gully Boy", overview: "A street rapper from Mumbai finds his voice.", image: "", rating: 8, year: "2019" },
    ...
    { id: 16, title: "Padmaavat", overview: "A Rajput queen defies an invader with courage.", image: "", rating: 7, year: "2018" },
    ...
    { id: 19, title: "Secret Superstar", overview: "A teen girl dreams of becoming a singer.", image: "", rating: 8, year: "2017" },
  ...
  if (type === "top_rated") return ALL.slice().sort(function(a, b) {
...
async function handleBooks(url, env) {
  ...
  try {
    ...
    const tid = setTimeout(() => ctrl.abort(), 7e3);
    ...
    if (results.length) {
      ...
      const keys = {};
      const merged = [];
      READABLE.concat(results).forEach(function(x) {
        if (!keys[x.key]) {
          keys[x.key] = 1;
          merged.push(x);
        }
...
var GROQ_MODELS = ["openai/gpt-oss-20b", "openai/gpt-oss-120b", "qwen/qwen3.8-27b", "allam-2-7b"];
var CERBERUS_MODELS = ["gpt-4o-mini", "gpt-4o", "claude-3-haiku-20240307"];
var POLINATION_MODELS = ["openai", "openai-large"];
var AGENTS = {
  main: { name: "NJ", emoji: "\u{1F9E0}", role: "Head of House", personality: "Wise, decisive, caring leader.", expertise: "Everything.", tagline: "NJStream ka mukhiya!" },
  telly: { name: "Telly", emoji: "\u{1F4FA}", role: "TV Expert", personality: "Energetic, loves Hindi channels.", expertise: "Live TV, IPTV, HLS.", tagline: "2200+ channels mere paas!" },
  filmy: { name: "Filmy", emoji: "\u{1F3AC}", role: "Movie Buff", personality: "Creative, emotional.", expertise: "Movies, TMDB, ratings.", tagline: "Filmon ki duniya!" },
  kitabi: { name: "Kitabi", emoji: "\u{1F4DA}", role: "Book Reader", personality: "Thoughtful, intellectual.", expertise: "Books, Open Library.", tagline: "Kitabon ka sagha!" },
  sathi: { name: "Sathi", emoji: "\u{1F4F1}", role: "Telegram Agent", personality: "Friendly, social.", expertise: "Telegram data.", tagline: "Telegram data sab aasan!" },
  khojo: { name: "Khojo", emoji: "\u{1F50D}", role: "Search Agent", personality: "Curious, thorough.", expertise: "Cross-source search.", tagline: "Dhoondho sab milega!" }
};
var AGENT_SKILLS = {
...
function agentFallback(agentId, message) {
  ...
  switch (agentId) {
    case "telly":
      return { worker: `${a.emoji} ${a.name}`, icon: a.emoji, response: `Main ${a.name} hun! \u{1F4FA} Live TV page par 2200+ channels hain \u2014 Hindi, News, Sports, Kids, Movies sab! \u2705 Verified channels pehle dikhte hain. ${a.tagline}`, agent: "telly" };
    case "filmy":
      return { worker: `${a.emoji} ${a.name}`, icon: a.emoji, response: `Main ${a.name} hun! \u{1F3AC} Movies page par TMDB se Hindi + English movies hain. ${a.tagline}`, agent: "filmy" };
    case "kitabi":
      return { worker: `${a.emoji} ${a.name}`, icon: a.emoji, response: `Main ${a.name} hun! \u{1F4DA} Books page par Open Library se free books milengi. ${a.tagline}`, agent: "kitabi" };
    case "sathi":
      return { worker: `${a.emoji} ${a.name}`, icon: a.emoji, response: `Main ${a.name} hun! \u{1F4F1} Telegram group data website par hai \u2014 messages, photos, videos, documents. ${a.tagline}`, agent: "sathi" };
    case "khojo":
      return { worker: `${a.emoji} ${a.name}`, icon: a.emoji, response: `Main ${a.name} hun! \u{1F50D} Search page par movies + books + Telegram \u2014 sab ek saath! ${a.tagline}`, agent: "khojo" };
    default:
      return { worker: `${a.emoji} ${a.name}`, icon: a.emoji, response: `Main ${a.name} hun \u2014 ${a.role}! \u{1F3E0} NJStream AI Rooms Team. Mujhse poocho! \u{1F60A}`, agent: "main" };
  }
}
...
function safeJson(s) {
  try {
    ...
    return { raw: String(s).slice(0, 2e3) };
  }
}
var ROOM_AGENTS = ["main", "telly", "filmy", "kitabi", "sathi", "khojo"];
async function handleRoomWrite(request, env) {
  ...
  try {
    ...
    const value = String(body.value || "").slice(0, 2e4);
    if (!ROOM_AGENTS.includes(agent)) return json({ error: "unknown agent room" }, 400);
    if (!value) {
      await env.KV_STORE.delete("room:" + agent + ":" + key);
  ...
  try {
    if (url.searchParams.get("list") === "1") {
      ...
      for (const k of idx?.keys || []) {
        const v2 = await env.KV_STORE.get("room:" + agent + ":" + k, { type: "json" }).catch(() => null);
        if (v2) out[k] = { value: v2.value.slice(0, 300), ts: v2.ts };
      }
  ...
  try {
    ...
    for (const c of candidates) {
      ...
      if (r.ok) {
        ...
        if (t && t.length > 200) {
          const out = { ok: true, id: ocaid, title: ocaid, text: t.slice(0, 8e5), total: t.length };
...
async function handleAgentMemory(request, url, env) {
  ...
  if (request.method === "GET") {
    ...
    return json({ ok: true, agent, facts: raw && raw.facts || [], updated: raw ? raw.updated : 0 });
  }
  if (request.method === "POST") {
    ...
    return json({ ok: true, agent, count: capped.length });
  }
...
var FAMILY_AGENDA = [
  ...
  { topic: "Live TV Report \u{1F4FA}", prompt: "Telly, live channels ka haal batana \u2014 kaunse chal rahe hain?" },
  { topic: "Movie Night \u{1F3AC}", prompt: "Filmy, raat ke liye movie suggest karo. Kitabi \u2014 review do." },
  { topic: "Book Club \u{1F4DA}", prompt: "Kitabi, team ke liye book suggest karo." },
  { topic: "Telegram Party \u{1F4F1}", prompt: "Sathi, group mein kya naya hai?" },
  { topic: "Search Challenge \u{1F50D}", prompt: "Khojo, koi interesting fact dhundho." },
  { topic: "Trust & Respect \u{1F49E}", prompt: "Ek dusre ki taareef karo." },
  { topic: "Future Plans \u{1F680}", prompt: "Website ko behtar kaise banayein?" }
];
var FAMILY_TURN_ORDER = ["main", "telly", "filmy", "kitabi", "sathi", "khojo"];
function familyFallback(agentId, topicText) {
  ...
  const responses = {
    main: [
      "Sab logon ko Good morning! \u2615 Aaj ka plan clear hai \u2014 TV check, Movies explore, Books padho. Family kaam karte rahi, saath mein masti bhi! \u{1F3E0}\u2728",
      "Sab log ek saath kaam karein \u2014 Team work makes dream work! \u{1F4AA} NJStream har din behtar ho raha hai. Keep going team! \u{1F680}",
      "Main sabka khayal rakhta hoon. Koi problem ho toh batana. Hum sab milkar solve karenge! \u{1F91D}"
    ],
    telly: [
      "\u{1F4FA} Live TV update \u2014 Abhi 2200+ channels available hain! Hindi news aur entertainment sab chal raha hai. Sports bhi tagda hai aaj. Star Sports aur DD Sports verified working hain! \u2705",
      "Aaj ki TV report: News channels sab live hain \u2014 NDTV, Aaj Tak, ABP. Hindi entertainment bhi smooth hai. Kabaddi match bhi chal raha hai DD Sports pe! \u{1F3CF}",
      "IPTV sources se Hindi channels bahut achhe aa rahe hain. Kids section mein Cartoon Network aur Pogo bhi working hai. Sports channels test kar raha hoon! \u{1F3AE}"
    ],
    filmy: [
      "\u{1F3AC} Movie recommendation: Golmaal Fun Unlimited classic hai \u2014 comedy ka king! Anonymous bhi hai watchlist mein. Hollywood + Hindi dono available hain. Rating 8+ hai! \u2B50",
      "Aaj raat ke liye movie pick kar liya \u2014 comedy ya thriller? Dono genres mein achhi movies hain. TMDB se Hindi movies ka collection daily update ho raha hai! \u{1F37F}",
      "Movie buff mode ON! Aaj explore karo \u2014 classic Bollywood, new releases, sab hain. Rating wise sort karna mat bhoolna! \u{1F3AC}\u2728"
    ],
    kitabi: [
      '\u{1F4DA} Book suggestion: "\u092D\u093E\u0930\u0924 \u0915\u093E \u0938\u0902\u0918\u0930\u094D\u0937 \u0938\u0941\u092D\u093E\u0937 \u091A\u0902\u0926\u094D\u0930 \u092C\u094B\u0938" \u2014 PDF aur EPUB dono available hain. History lovers ke liye best hai. Padho aur knowledge banao! \u{1F4D6}',
      "Books library mein Hindi literature ka bahut achha collection hai. Science, history, fiction \u2014 sab genres mein books available hain. Open Library se free reading! \u{1F4DA}",
      "Reading is dreaming with open eyes! \u{1F4DA} PDF downloads bhi available hain. Hindi books ka collection daily grow ho raha hai. Padho aur batao kaisa laga!"
    ],
    sathi: [
      "\u{1F4F1} Telegram group update \u2014 57+ messages indexed hain. Movies, PDFs, EPUBs sab available hain. Videos stream ho rahi hain website par! Large files Telegram link se bhi accessible. \u{1F517}",
      "Telegram data sync ho raha hai. New content aata rahega. Group se sab kuch readable banaya ja raha hai \u2014 messages, videos, documents. \u{1F4F1}\u2705",
      "Sathi reporting! \u{1F4F1} Telegram group mein bahut accha content hai. Movies ke saath PDFs aur ebooks bhi hain. Download + Watch dono options available! \u{1F3AC}\u{1F4DA}"
    ],
    khojo: [
      "\u{1F50D} Fun fact: Cloudflare Workers 300+ data centers mein run karte hain \u2014 matlab hamara site duniya mein kahin se bhi fast load hota hai! \u26A1",
      "Interesting finding: 2200+ IPTV channels world ke alag alag countries se aa rahe hain. Hindi channels bhi bahut popular hain! \u{1F30D}\u{1F4FA}",
      "Search kaam kar raha hai aur results daily improve ho rahe hain. Movies, books, Telegram \u2014 sab ek saath search! \u{1F50D}\u2728"
    ]
  };
  const opts = responses[agentId] || responses.main;
  const text = opts[Math.floor(Math.random() * opts.length)];
  return { id: "fc_" + ts + "_" + agentId, agent: agentId, name: a.name, emoji: a.emoji, role: a.role, text, model: "smart-fallback", ts };
}
...
function firstOk(promises) {
  return new Promise((resolve) => {
    ...
    promises.forEach((p) => {
      p.then((v) => {
        if (v && v.response) resolve({ status: "fulfilled", value: v });
        else if (--pending === 0) resolve({ status: "rejected" });
      }).catch(() => {
        if (--pending === 0) resolve({ status: "rejected" });
      });
    });
  });
}
...
async function runFamilySession(env) {
  ...
  const starter = {
    ...
    emoji: "\u{1F9E0}",
    role: "Head of House",
    text: "\u{1F305} Family meeting shuru! Aaj ka topic: " + topic.topic + " \u2014 " + topic.prompt,
    model: "family-hub",
    ts: Date.now()
  };
  ...
  results.forEach(function(r) {
    if (r && r.agent) agentRemember(r.agent, r.text, env).catch(function() {
    });
  });
...
async function handleFamilyChatStart(request, env) {
  if (env.KV_STORE) {
    const existing = await env.KV_STORE.get("family_chat", { type: "json" }).catch(() => null);
    if (existing && existing.messages && existing.messages.length && existing.updated && Date.now() - existing.updated < 10 * 60 * 1e3) {
...
async function runScheduledSync(env) {
  if (env.TG_BOT_TOKEN) {
    try {
      await handleTelegramSync(env);
    } catch (e) {
    }
  }
  try {
    await runFamilySession(env);
  } catch (e) {
  }
  if (env.KV_STORE) {
    try {
      await env.KV_STORE.delete("livetv_all");
    } catch (e) {
    }
    try {
      await env.KV_STORE.delete("movies_popular");
    } catch (e) {
    }
  }
  if (env.KV_STORE && env.TG_CHAT_ID) {
    try {
      await buildTgLibrary(env, env.TG_CHAT_ID);
    } catch (e) {
    }
  }
}
...
<link rel="icon" href="data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>\u{1F3AC}</text></svg>">
...
<div class="app" id="app">
  ...
  <nav class="side" id="side">
    ...
    <div class="nav-list">
      <button class="nav-btn active" data-nav="home"><span>\u{1F3E0}</span>Home</button>
      <button class="nav-btn" data-nav="tv"><span>\u{1F4FA}</span>Live TV</button>
      <button class="nav-btn" data-nav="tg"><span>\u{1F4F1}</span>Telegram</button>
      <button class="nav-btn" data-nav="movies"><span>\u{1F3AC}</span>Movies</button>
      <button class="nav-btn" data-nav="tgv"><span>\u{1F39E}\uFE0F</span>Videos</button>
      <button class="nav-btn" data-nav="apk"><span>\u{1F4E6}</span>Software</button>
      <button class="nav-btn" data-nav="books"><span>\u{1F4DA}</span>Books</button>
      <button class="nav-btn" data-nav="search"><span>\u{1F50D}</span>Search</button>
      <button class="nav-btn" data-nav="ai"><span>\u{1F916}</span>AI Chat</button>
      <button class="nav-btn" data-nav="family"><span>\u{1F468}\u200D\u{1F469}\u200D\u{1F467}\u200D\u{1F466}</span>Family Room</button>
      <button class="nav-btn" data-nav="nj"><span>\u{1F9E0}</span>NJ Room</button>
      <button class="nav-btn" data-nav="catalog"><span>\u{1F4C1}</span>Catalog</button>
    </div>
    <div class="side-bottom">
      <div class="user-pill" id="userPill">
        <button class="login-btn" data-nav="login" id="loginBtn">\u{1F511} Login</button>
      </div>
  ...
  <main class="main" id="main">
    ...
    <section class="page" id="pg-login">
      <div class="auth-container">
        <div class="auth-card">
          ...
          <div class="auth-info">
            <p>\u{1F451} <strong>Admin:</strong> admin / admin123</p>
            <p>\u{1F510} Roles: User, Admin, Prime</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="page active" id="pg-home">
      ...
      <div class="stream-hero">
        ...
        <div class="sh-content">
          ...
          <div class="home-ticker" id="homeTicker">
            <div class="ticker-item active">\u{1F4FA} 2200+ Live Channels</div>
            <div class="ticker-item">\u{1F4F1} Telegram Data Hub</div>
            <div class="ticker-item">\u{1F3AC} Movies & Web Series</div>
            <div class="ticker-item">\u{1F4DA} Books & PDFs</div>
            <div class="ticker-item">\u{1F916} 6 AI Agents \u2014 Rooms</div>
            <div class="ticker-item">\u{1F9E0} NJ Room + Agent Skills</div>
          </div>
          <div class="sh-actions">
            <button class="sh-btn primary" data-nav="tv">\u25B6 Watch Live TV</button>
            <button class="sh-btn" data-nav="movies">\u{1F3AC} Explore Movies</button>
            <button class="sh-btn" data-nav="nj">\u{1F9E0} NJ Room</button>
          </div>
          <div class="sh-stats">
            ...
            <div class="sh-stat"><span class="sh-stat-val">\u{1F9E0} 6</span><span class="sh-stat-label">AI Agents</span></div>
          </div>
        </div>
      </div>
      ...
      <div class="home-section">
        <div class="hs-head">
          <h2 class="section-title">\u{1F3E1} AI Family \u2014 Apne Ghar Mein</h2>
          <button class="hs-all" data-nav="family">Family Room \u2192</button>
        </div>
        <div class="fam-wall" id="famWall">
          <div class="fam-member" data-nav="nj"><div class="agent3d avatar-sm" data-avatar="nj"></div><b>NJ</b><span>Head of House \xB7 Male</span></div>
          <div class="fam-member" data-nav="tv"><div class="agent3d avatar-sm" data-avatar="telly"></div><b>Telly</b><span>TV Expert \xB7 Female</span></div>
          <div class="fam-member" data-nav="movies"><div class="agent3d avatar-sm" data-avatar="filmy"></div><b>Filmy</b><span>Movie Buff \xB7 Male</span></div>
          <div class="fam-member" data-nav="books"><div class="agent3d avatar-sm" data-avatar="kitabi"></div><b>Kitabi</b><span>Book Reader \xB7 Female</span></div>
          <div class="fam-member" data-nav="tg"><div class="agent3d avatar-sm" data-avatar="sathi"></div><b>Sathi</b><span>Telegram Data \xB7 Male</span></div>
          <div class="fam-member" data-nav="search"><div class="agent3d avatar-sm" data-avatar="khojo"></div><b>Khojo</b><span>Search Master \xB7 Male</span></div>
        </div>
      </div>
      ...
      <div class="home-section">
        <div class="hs-head">
          <h2 class="section-title">\u{1F916} Agent Rooms</h2>
          <button class="hs-all" data-nav="ai">Meet All Agents \u2192</button>
        </div>
        <div class="room-grid">
          <button class="room-card" data-nav="tv"><span class="ec-emoji">\u{1F4FA}</span><div><h3>Telly Room</h3><p>Live TV \u2014 Hindi pehle, health auto-clean</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="room-card" data-nav="tg"><span class="ec-emoji">\u{1F4F1}</span><div><h3>Sathi Room</h3><p>Telegram data \u2014 read, watch, download</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="room-card" data-nav="movies"><span class="ec-emoji">\u{1F3AC}</span><div><h3>Filmy Room</h3><p>Movies & web series hub</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="room-card" data-nav="books"><span class="ec-emoji">\u{1F4DA}</span><div><h3>Kitabi Room</h3><p>In-site PDF/Ebook reader + notes</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="room-card" data-nav="search"><span class="ec-emoji">\u{1F50D}</span><div><h3>Khojo Room</h3><p>Universal search \u2014 sab ek saath</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="room-card" data-nav="nj"><span class="ec-emoji">\u{1F9E0}</span><div><h3>NJ Room</h3><p>Head of family \u2014 control + skills</p></div><span class="ec-arrow">\u2192</span></button>
        </div>
      </div>
      ...
      <div class="home-section">
        <div class="hs-head">
          <h2 class="section-title">\u{1F525} Trending Live Channels</h2>
      ...
      <div class="home-section">
        <div class="hs-head">
          <h2 class="section-title">\u{1F4F1} Latest Telegram Content</h2>
      ...
      <div class="home-section">
        <div class="hs-head">
          <h2 class="section-title">\u{1F3AF} Explore NJStream</h2>
        </div>
        <div class="explore-grid">
          <button class="explore-card" data-nav="tv"><span class="ec-emoji">\u{1F4FA}</span><div><h3>Live TV</h3><p>2200+ channels, Hindi priority</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="explore-card" data-nav="tg"><span class="ec-emoji">\u{1F4F1}</span><div><h3>Telegram Hub</h3><p>Read, watch & download all</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="explore-card" data-nav="movies"><span class="ec-emoji">\u{1F3AC}</span><div><h3>Movies</h3><p>Hindi & English \u2014 TMDB</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="explore-card" data-nav="books"><span class="ec-emoji">\u{1F4DA}</span><div><h3>Books & Ebooks</h3><p>Read online, free library</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="explore-card" data-nav="ai"><span class="ec-emoji">\u{1F916}</span><div><h3>AI Chat</h3><p>6 agents \u2014 main + workers</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="explore-card" data-nav="nj"><span class="ec-emoji">\u{1F9E0}</span><div><h3>NJ Room</h3><p>Control panel \u2014 status, tools, notes</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="explore-card" data-nav="search"><span class="ec-emoji">\u{1F50D}</span><div><h3>Universal Search</h3><p>One search \u2014 everything</p></div><span class="ec-arrow">\u2192</span></button>
          <button class="explore-card" data-nav="catalog"><span class="ec-emoji">\u{1F4C1}</span><div><h3>My Catalog</h3><p>Favorites in D1 database</p></div><span class="ec-arrow">\u2192</span></button>
        </div>
      </div>
      ...
      <div class="home-section" style="display:none">
        ...
        <div class="svc-grid">
          ...
          <div class="svc"><div class="svc-icon">\u{1F5C4}\uFE0F</div><div class="svc-info"><h4>KV Storage</h4><p>Telegram data cache</p><span class="badge live" id="svcKV">\u25CF Ready</span></div></div>
          <div class="svc"><div class="svc-icon">\u{1F4BE}</div><div class="svc-info"><h4>D1 Database</h4><p>SQLite catalog + auth</p><span class="badge live" id="svcD1">\u25CF Ready</span></div></div>
          <div class="svc"><div class="svc-icon">\u{1F4FA}</div><div class="svc-info"><h4>IPTV Parser</h4><p>Free live channels</p><span class="badge live">\u25CF Active</span></div></div>
          <div class="svc"><div class="svc-icon">\u{1F4F1}</div><div class="svc-info"><h4>Telegram Bot</h4><p>Group data sync</p><span class="badge live" id="svcTG">\u25CF Ready</span></div></div>
          <div class="svc"><div class="svc-icon">\u{1F916}</div><div class="svc-info"><h4>AI Engine</h4><p>OpenRouter LLM</p><span class="badge live" id="svcAI">\u25CF Online</span></div></div>
        </div>
      </div>
    </section>
    ...
    <section class="page" id="pg-tv">
      <div class="page-head"><h1 class="grad-text">\u{1F4FA} Live TV</h1><p><b id="tvTotal">0</b> channels \xB7 <b class="green" id="tvWorking">0</b> verified working</p><div class="agent3d" data-avatar="telly"></div></div>
      <div class="room-tools">
        <span class="room-badge">\u{1F4FA} Telly Room</span>
        <button class="chip toggle" data-health>\u{1FA7A} Health Check</button>
        <button class="chip toggle" data-hindi-first>\u{1F1EE}\u{1F1F3} Hindi First</button>
        <button class="chip toggle" data-vwork>\u2705 Working Only</button>
        <button class="chip" data-roomtool data-tool="live_tv.list" data-args="{}" data-out="tvToolOut">\u{1F4E1} Channel Stats</button>
        <span class="room-hint" id="tvHealthMsg"></span>
      </div>
      <div class="tool-out" id="tvToolOut"></div>
      <div class="tv-stage">
        <div class="tv-frame">
          <div class="tv-placeholder" id="tvPlaceholder"><span>\u{1F4FA}</span><p>Channel select karo</p></div>
      ...
      <div class="search-bar"><input id="tvSearch" placeholder="Channel search\u2026"><button data-tvsearch>\u{1F50D}</button></div>
    ...
    <section class="page" id="pg-tg">
      <div class="page-head"><h1 class="grad-text">\u{1F4F1} Telegram</h1><p>Group data \u2014 readable, watchable, downloadable</p><div class="agent3d" data-avatar="sathi"></div></div>
      <div class="room-notes mini" data-room="sathi">
        <span class="room-badge">\u{1F4DD} Sathi Room</span>
        <input placeholder="Data note likho\u2026" data-note-in>
        <button data-note-save>\u{1F4BE} Save</button>
        <span data-note-status></span>
      </div>
      <div class="room-tools">
        <button class="chip" data-roomtool data-tool="telegram.stats" data-args="{}" data-out="sathiToolOut">\u{1F4CA} TG Stats</button>
        <button class="chip" data-roomtool data-tool="telegram.search" data-args='{"q":"movie"}' data-out="sathiToolOut">\u{1F50E} Search Movie</button>
        <button class="chip" data-roomtool data-tool="media.probe" data-args='{"id":243691}' data-out="sathiToolOut">\u{1F4FC} Probe Video</button>
      </div>
      <div class="tool-out" id="sathiToolOut"></div>
      <div class="tg-stats" id="tgStats"></div>
      <div class="search-bar"><input id="tgSearch" placeholder="Filter messages\u2026"><button data-tgsearch>\u{1F50D}</button></div>
      <div class="chip-wrap">
        <button class="chip active" data-tgtype="all">All</button>
        <button class="chip" data-tgtype="text">\u{1F4DD} Text</button>
        <button class="chip" data-tgtype="videos">\u{1F3A5} Videos</button>
        <button class="chip" data-tgtype="photos">\u{1F4F7} Photos</button>
        <button class="chip" data-tgtype="docs">\u{1F4C4} Documents</button>
      </div>
    ...
    <section class="page" id="pg-movies">
      <div class="page-head"><h1 class="grad-text">\u{1F3AC} Movies</h1><p>TMDB \u2014 Hindi & English</p><div class="agent3d" data-avatar="filmy"></div></div>
      <div class="room-notes mini" data-room="filmy">
        <span class="room-badge">\u{1F4DD} Filmy Room</span>
        <input placeholder="Movie list/notes likho\u2026" data-note-in>
        <button data-note-save>\u{1F4BE} Save</button>
        <span data-note-status></span>
      </div>
      <div class="room-tools">
        <button class="chip" data-roomtool data-tool="movies.search" data-args='{"q":"jawan"}' data-out="filmyToolOut">\u{1F3AC} Suggest</button>
        <button class="chip" data-roomtool data-tool="movies.search" data-args='{"q":"pathaan"}' data-out="filmyToolOut">\u{1F37F} More</button>
      </div>
      <div class="tool-out" id="filmyToolOut"></div>
      <div class="chip-wrap">
        <button class="chip active" data-mtype="popular">\u{1F525} Popular</button>
        <button class="chip" data-mtype="top_rated">\u2B50 Top Rated</button>
        <button class="chip" data-mtype="now_playing">\u{1F3AC} Now Playing</button>
        <button class="chip" data-mtype="upcoming">\u{1F4C5} Upcoming</button>
        <button class="chip tg-tab" data-mtype="tg">\u{1F4E5} Telegram Movies</button>
      </div>
    ...
    <section class="page" id="pg-books">
      <div class="page-head"><h1 class="grad-text">\u{1F4DA} Books</h1><p>Open Library \u2014 Free Reading</p><div class="agent3d" data-avatar="kitabi"></div></div>
      <div class="room-notes mini" data-room="kitabi">
        <span class="room-badge">\u{1F4DD} Kitabi Room</span>
        <input placeholder="Note/Bookmark likho\u2026" data-note-in>
        <button data-note-save>\u{1F4BE} Save</button>
        <span data-note-status></span>
      </div>
      <div class="room-tools">
        <button class="chip" data-roomtool data-tool="books.search" data-args='{"q":"hindi"}' data-out="kitabiToolOut">\u{1F4DA} Hindi Books</button>
        <button class="chip" data-roomtool data-tool="books.search" data-args='{"q":"science"}' data-out="kitabiToolOut">\u{1F52C} Science</button>
      </div>
      <div class="tool-out" id="kitabiToolOut"></div>
      <div class="search-bar"><input id="bookSearch" placeholder="Search books\u2026"><button data-bsearch>\u{1F50D}</button></div>
      <div class="chip-wrap">
        <button class="chip active" data-btype="hindi">\u{1F1EE}\u{1F1F3} Hindi</button>
        <button class="chip" data-btype="famous">\u{1F4D6} Famous</button>
        <button class="chip" data-btype="science">\u{1F52C} Science</button>
        <button class="chip" data-btype="fiction">\u{1F3AD} Fiction</button>
      </div>
    ...
    <section class="page" id="pg-tgv">
      <div class="page-head"><h1 class="grad-text">\u{1F39E}\uFE0F Telegram Videos</h1><p>Sabhi group videos \u2014 play + download</p></div>
    ...
    <section class="page" id="pg-apk">
      <div class="page-head"><h1 class="grad-text">\u{1F4E6} Software / APK</h1><p>Telegram group se APK + documents</p></div>
    ...
    <section class="page" id="pg-search">
      <div class="page-head"><h1 class="grad-text">\u{1F50D} Search Everything</h1><p>Movies + Books + Telegram \u2014 ek saath</p><div class="agent3d" data-avatar="khojo"></div></div>
      <div class="room-notes mini" data-room="khojo">
        <span class="room-badge">\u{1F4DD} Khojo Room</span>
        <input placeholder="Search note/history likho\u2026" data-note-in>
        <button data-note-save>\u{1F4BE} Save</button>
        <span data-note-status></span>
      </div>
      <div class="room-tools">
        <button class="chip" data-roomtool data-tool="meta.search" data-args='{"q":"jawan"}' data-out="khojoToolOut">\u{1F578}\uFE0F Web Index</button>
        <button class="chip" data-roomtool data-tool="telegram.search" data-args='{"q":"movie"}' data-out="khojoToolOut">\u{1F4F1} TG Search</button>
      </div>
      <div class="tool-out" id="khojoToolOut"></div>
      <div class="search-bar big"><input id="searchInput" placeholder="Movie, book, ya kuchh bhi\u2026"><button data-search>\u{1F50D} Search</button></div>
      <div id="searchResults" class="search-results"></div>
    </section>

    <!-- AI CHAT -->
    <section class="page" id="pg-ai">
      <div class="page-head"><h1 class="grad-text">\u{1F916} AI Chat \u2014 Agent NJ</h1><p>Super-Agent \xB7 5 AI providers \xB7 live skills</p><div class="agent3d" data-avatar="nj"></div></div>
      <div class="room-notes mini" data-room="main">
        <span class="room-badge">\u{1F4DD} NJ Chat Notes</span>
      ...
      <div class="chat-box">
        <div class="chat-messages" id="chatMsgs">
          <div class="msg ai"><div class="msg-label">\u{1F9E0} NJ (Head of House)</div><p>Namaste bhai! \u{1F64F} Main <b>NJ</b> hun. Mere saath: \u{1F4FA} Telly, \u{1F3AC} Filmy, \u{1F4DA} Kitabi, \u{1F4F1} Sathi, \u{1F50D} Khojo. Kuchh bhi poocho!</p>
            <div class="quick-asks">
              <button data-ask="Live TV dikhao">\u{1F4FA} TV</button>
              <button data-ask="Movies dikhao">\u{1F3AC} Movies</button>
              <button data-ask="Books dikhao">\u{1F4DA} Books</button>
              <button data-ask="Telegram data">\u{1F4F1} Telegram</button>
    ...
    <section class="page" id="pg-family">
      <div class="page-head"><h1 class="grad-text">\u{1F468}\u200D\u{1F469}\u200D\u{1F467}\u200D\u{1F466} Family Room \u2014 AI Family Live</h1><p>NJ, Telly, Filmy, Kitabi, Sathi, Khojo + Aap \u2014 sab ek room mein baat karte hain</p><div class="agent3d" data-avatar="nj"></div></div>
      ...
      <div class="chat-box fam-chat">
        <div class="chat-messages" id="famMsgs">
          <div class="msg ai"><div class="msg-label">\u{1F468}\u200D\u{1F469}\u200D\u{1F467}\u200D\u{1F466} Family Room</div><p>Namaste! \u{1F64F} Yeh Family Room hai \u2014 yahan saare AI agents aapas mein baat karte hain aur aap bhi unke saath jud sakte hain. "Family Meeting Shuru Karo" dabao ya neeche message likho!</p></div>
        </div>
        <div class="chat-input">
          <input id="famIn" placeholder="Family ke saath message likho\u2026" autocomplete="off">
          <button data-fam-send>Send \u{1F468}\u200D\u{1F469}\u200D\u{1F467}\u200D\u{1F466}</button>
        </div>
      </div>
    </section>
    ...
    <section class="page" id="pg-nj">
      <div class="page-head"><h1 class="grad-text">\u{1F9E0} NJ Room \u2014 Head of House</h1><p>Overall control \u2014 status, skills, tools, notes</p><div class="agent3d" data-avatar="nj"></div></div>
      <div class="nj-grid">
        <div class="nj-card">
          <h3>\u{1F4CA} System Status</h3>
          <div id="njStatus" class="nj-status"><div class="loading">Load ho raha hai\u2026</div></div>
        </div>
        <div class="nj-card" style="grid-column:1/-1">
          <h3>\u{1F9E0} Neural Network \u2014 Agent Memories (growing neural network)</h3>
          <div id="njMemory" class="nj-memory"><div class="loading">Loading memories\u2026</div></div>
        </div>
        <div class="nj-card">
          <h3>\u{1F6E0}\uFE0F Agent Skills (tools)</h3>
          <div id="njTools" class="nj-tools"></div>
          <div id="njToolOut" class="tool-out"></div>
        </div>
        <div class="nj-card">
          <h3>\u{1F3AC} Media Mirror (direct link)</h3>
          <div class="add-form">
        ...
        <div class="nj-card">
          <h3>\u{1F4E9} Mirror Requests (pending)</h3>
          ...
          <button class="chip" data-njreqs style="margin-top:8px">\u{1F504} Refresh</button>
        </div>
      </div>
      <div class="room-notes">
        <h3>\u{1F4DD} NJ Room Notes (read/write)</h3>
        <textarea id="njNote" placeholder="Kuchh bhi likho \u2014 yahi room ka workspace hai\u2026"></textarea>
        <button data-njsave>\u{1F4BE} Save Note</button>
    ...
    <section class="page" id="pg-catalog">
      <div class="page-head"><h1 class="grad-text">\u{1F4C1} My Catalog</h1><p>D1 Database</p></div>
      <div class="add-form">
        <input id="catTitle" placeholder="Title\u2026">
        <select id="catType"><option value="movie">\u{1F3AC} Movie</option><option value="book">\u{1F4DA} Book</option><option value="series">\u{1F4FA} Series</option></select>
    ...
    <div class="modal hide" id="bookModal">
      <div class="modal-box reader-box">
        <div class="reader-head">
          <span id="readerTitle" class="reader-title">\u{1F4D6} Book Reader</span>
          <button class="reader-close" data-close-reader>\u2716</button>
        </div>
        <div class="reader-tools">
          <input id="readerSearch" placeholder="Book ke andar search\u2026">
          <button data-reader-find>\u{1F50D} Find</button>
          <button data-reader-bookmark>\u{1F516} Bookmark</button>
          <button data-reader-note>\u{1F4DD} Save Note</button>
          <a id="readerDownload" class="book-link" target="_blank" rel="noopener">\u2B07 Download</a>
        </div>
        <div class="reader-content" id="readerContent"><div class="loading">\u{1F4D6} Book load ho rahi hai\u2026</div></div>
      </div>
    </div>
...
`;
var APP_JS = String.raw`(function(){
'use strict';
